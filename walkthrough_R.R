...
pmwg_args <- list(
  "burn_particles" = 1000,
  "burn_iter" = 500,
  "adapt_particles" = 100, #GC: should be 1000
  "adapt_maxiter" = 5000,  #GC: David code has max 100,000 across all iter types
...
#GC: ^ Gets num_par (same as num_randeffect), sets v_half, A_half, num subjects, max_iter (sum of all other iters)
#GC: initialises latent_theta_mu (7x19xmax_iter), param_theta_mu (7xmax_iter), param_theta_sigma2 (7x7xmax_iter)
#GC: k_half, v_shape, a_half also calculated and returned
start_points_mu <- c(.2, .2, .2, .4, .3, 1.3, -2)  #GC: start points not as specific as David's
start_points_sig2 <- diag(rep(.01, init$num_par))  #GC: 0.01 on diagonal, elsewhere 0, David has start points generated from inverse wishart
...
particles <- array(dim = c(length(ptm), init$S))  #GC: Empty array dim 7x19
...
#GC: particles after this loop seems to be roughly similar to theta_latent after
# line 36 of matlab file, however variance across subject in theta_latent seems higher.
for (s in 1:init$S) {
