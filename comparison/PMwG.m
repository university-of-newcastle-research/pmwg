% GC: COMPARISON of R code and MATLAB code

% GC: No MATLAB equivalent of loading R libraries or clearing workspace
%
%
%
%
%
%------------------------------------------------------------------------
%% GC: Load Forstmann et al.'s data.
load('LBA_firstthree.mat'); %load the dataset, see an example in the 'LBA_realdata.mat'
% GC: Data formatted to already have column names
num_subjects=length(data.rt); %number of subjects in the experiments
%------------------------------------------------------------------------
%% GC: Extract numbers of trials per subject
for j=1:num_subjects
    num_trials(j,1)=length(data.rt{j,1}); %computing the number of trials per subject
end
%------------------------------------------------------------------------
%% GC: No ability to restart
%
%parpool(28) %number of processors available to be used. (GC: orginally commented in code)
%------------------------------------------------------------------------
%% GC: Set PMwG process "parameters"
...
%initial values of the hyperparameters for lower level parameters (GC: UNNAMED HERE)
num_randeffect=7; %the total number of random effects in the LBA model. For Forstmann dataset, we have 7 random effects.
num_choice=2; % the number of response options in the task (GC: Not used elsehere?)
...
prior_mu_sig2=eye(num_randeffect);%the prior for \Sigma_{\alpha} eye is identity matrix
% GC: Apparently hyperparameters are tuning settings
v_half=2; %the hyperparameters of the prior of \Sigma_{\alpha}
A_half=1; %the hyperparameters of the prior of \Sigma_{\alpha}
%------------------------------------------------------------------------
%% GC: Sample storage and start point
% No sample storage set up here, this is created "on the fly" during iteration
param.theta_mu=[0.2695;0.2116;-0.0241;-0.4017;0.2978;1.1230;-1.7128]; %the initial values for parameter \mu
% GC: MATLAB code does an inverse wishart for the theta.sig2
param.theta_sig2=iwishrnd(eye(num_randeffect),20); % the initial values for \Sigma - inverse wishart random number
...
%------------------------------------------------------------------------
%% GC: No obvious equivalent for R specific steps here
%
%
%
%
...
%
%------------------------------------------------------------------------
%% GC: Sample initial values for random effects
[theta_latent]=LBA_MC_v1(data,param,num_subjects,num_trials,num_particles); %obtain initial values of the random effects using Monte Carlo method
% Responsibility passed on to separate matlab file/function
%
%
%
%
...
%initial values for a_{1},...,a_{7}
a_half=1./random('gam',1/2,1,num_randeffect,1); 
...
i=1;
%------------------------------------------------------------------------
%% GC: No equivalent iteration function here
%
%
%
%
...
%
%------------------------------------------------------------------------
%% GC: Matlab parallel setup is not done
%
%
%
%
...
while i<=s
    ...
    chol_var_mu=chol(var_mu,'lower');  %Cholesky factorization - MATLAB chol - MathWorks
    param.theta_mu=mvnrnd(mean_mu,chol_var_mu*chol_var_mu');
	% GC: Matlab has no equivalent for names here
%------------------------------------------------------------------------
...
%------------------------------------------------------------------------
%% GC: Conditional Monte Carlo - select better proposal if we are getting enough unique random effects
...
%------------------------------------------------------------------------
%% after burn in, we count the number of unique values in the random effects for each subject.
    if i>burn
        for j=1:num_subjects
...
%------------------------------------------------------------------------
%% GC: Save temp progress occasionally as we iterate, and end loop
...
save('LBA_firstthree_output.mat','theta_mu_store','theta_sig2_store1','theta_sig2_store2','theta_sig2_store3','theta_sig2_store4',...
    'theta_sig2_store5','theta_sig2_store6','theta_sig2_store7','theta_latent_b1_store','theta_latent_b2_store','theta_latent_b3_store',...
    'theta_latent_A_store','theta_latent_v1_store','theta_latent_v2_store','theta_latent_tau_store',...
